...
- The next step is `/nxs:dev-plan` (turn the brief into a plan). For a bug rather than a task, route to `/nxs:dev-diagnose` instead of brainstorming.
...
- `/nxs:rnd-brainstorm` - two phases, CLARIFY (remove scope / terminology ambiguity) + EXPLORE (compare approaches) - open task shaping.
- `/nxs:rnd-dialectic` - compare two specific approaches head to head.
- `/nxs:rnd-rethink` - stop the current approach and find an alternative.

## NEXT

Brief written -> `/nxs:dev-plan` turns it into a plan following the `plan-conventions` contract, then `/nxs:dev-exec` to implement. For a bug rather than a task -> `/nxs:dev-diagnose`. For a plain-language statement of the brief -> `/nxs:std-explain`.

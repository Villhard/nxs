...
Loaded on demand from `/nxs:dev-review`. Carries the full review verdict contract - BLOCK / NIT / DROP classification, the anti-hypothetical filter, the high-confidence threshold, the pre-emit check, ranking, the finding output format, and the Standards / Spec axis classification tables. The operational rules live in SKILL.md; this file is the detail.
...
1. A command argument (`/nxs:dev-review <path>` points to a plan / brief / spec).

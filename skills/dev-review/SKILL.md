...
- Fixing is the user or `/nxs:dev-exec`. This skill reports and stops.
...
1. Command argument (`/nxs:dev-review <path>` points to a plan / brief / spec).
...
Findings to address -> hand them back to the user or to `/nxs:dev-exec` to fix, then re-review the same scope. Clean approve -> proceed to commit / MR.

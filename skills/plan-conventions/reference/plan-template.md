...
Task completion is tracked by flipping checkboxes `- [ ]` -> `- [x]` inside the plan file; `/nxs:exec` updates them after a task is done. Moving the plan to `docs/plans/completed/` happens separately, after explicit user confirmation - not automatically on the last checkbox.

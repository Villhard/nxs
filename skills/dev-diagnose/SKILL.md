...
- No fix, patch, or code change here. No exceptions. The fix goes through `/nxs:dev-plan` (fix plan) and `/nxs:dev-exec` (execution).
...
Root cause confirmed -> `/nxs:dev-plan` for the fix plan, then `/nxs:dev-exec` to implement. For a plain-language statement of the cause first -> `/nxs:std-explain`.

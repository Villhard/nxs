...
Loaded on demand from `/nxs:dev-exec` when the plan sets `Development approach: TDD`. The cycle is per behavior, not per task: one task / vertical slice = one or more short RED -> GREEN -> REFACTOR cycles. Common to default and auto modes.

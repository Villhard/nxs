---
description: Create an implementation plan that becomes the source of truth for execution - decompose a task, brief, or Jira input into sequenced, verifiable vertical-slice tasks. Use before executing non-trivial work, after a brainstorm or diagnosis, or whenever you need a reviewable plan.
argument-hint: "[task | brief path | jira key]"
---

# /nxs:dev-plan

Turn a task, brief, or Jira input into a well-formed implementation plan and stop. The plan becomes the source of truth for execution. Self-contained skill. Output language and response style come from global rules, not this file.

Accepted input: a task description, a brainstorm brief (`docs/briefs/`), a root-cause brief from `/nxs:dev-diagnose`, or a Jira key / URL / pasted ticket. A plan can be made with or without a brief - with no input, gather it here first.
...
- The plan is a proposal: read-only until the user approves it. Review is a separate step (`/nxs:dev-plan-review`); execution is a separate step (`/nxs:dev-exec`).
- If the task is small and single-step, no plan is needed - route to `/nxs:dev-exec` or a direct edit instead of ceremony.

## INTAKE

- If the input is a Jira key, URL, or pasted ticket, run the `jira-intake` background skill first to parse structure and separate facts from assumptions, then continue here.
...
1. Understand scope. Parse intent - feature, bug fix, refactor, migration, or generic. Pull acceptance criteria from the Jira / brief / root-cause brief; if none exist, formulate them.
...
Plan written -> `/nxs:dev-plan-review` to review it before execution, then `/nxs:dev-exec` to implement. For a plain-language statement of what the plan does -> `/nxs:std-explain`.

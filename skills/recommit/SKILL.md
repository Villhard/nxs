...
- EXPLICIT-ONLY - runs only when the user asks. Never auto-invoked, never run in auto or background mode. If reached from an auto/background context, refuse and tell the user to call `/nxs:recommit` directly.
...
History cleaned -> open the PR yourself (this skill never pushes). For a standalone code review of the branch -> `/nxs:review`. To archive finished plans / stale briefs (docs, not git history) -> `/nxs:clean`.

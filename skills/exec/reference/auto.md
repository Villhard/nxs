...
Loaded on demand from `/nxs:exec` when running `auto`. Carries the complete auto cycle, the exhaustive stop-condition and never-do lists, the no-commit override, out-of-scope handling, and cap / stalemate detection. `auto` is a reserved mode word and is never inferred from context.
...
- **plan-review recommended** - if `/nxs:plancheck` was not run, ask once and do not insist twice.
...
> "Plan-review was not run. Run /nxs:plancheck before auto mode or continue without it?"

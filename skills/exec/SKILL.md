...
Default task done -> review the findings, commit, then run `/nxs:exec` again for the next task, or `/nxs:exec auto` for the rest. For a standalone code review -> `/nxs:review`. To archive a finished plan (only on your confirmation) -> `/nxs:clean`.

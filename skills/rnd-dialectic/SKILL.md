...
Pick the mode from the input. If only a fuzzy direction exists and the options are not yet concrete, this is not dialectic - route to `/nxs:rnd-brainstorm` first.
...
- Options not yet formulated, still exploring the shape of the solution -> `/nxs:rnd-brainstorm`.
- A design question that depends on runtime behavior you can observe -> `/nxs:rnd-prototype`.
- Structural friction with no two options to weigh -> `/nxs:rnd-architecture`.

---
description: Read-only multi-lens review of an implementation plan before execution, especially before auto modes - four reviewer lenses report BLOCK / NIT findings without editing the plan. Use after /nxs:dev-plan and before /nxs:dev-exec, and always before /nxs:dev-exec auto.
...
Use before `/nxs:dev-exec`, and especially before `/nxs:dev-exec auto`, where the bar is higher.
...
- Fixing the plan is the user or `/nxs:dev-plan`. Executing it is `/nxs:dev-exec`. This skill reports and stops.
...
If the plan is headed for `/nxs:dev-exec auto`:
...
Plan clean -> `/nxs:dev-exec` (or `/nxs:dev-exec auto`) to implement. Findings to fix -> `/nxs:dev-plan` to revise the plan, then re-review.

...
The backbone that survives between sessions: 3 to 5 upcoming nodes, each tied to one line of
...
- **One line per node**, numbered, 3 to 5 nodes at a time. A queue, not a syllabus.
- **No dates, no reasons, no history.** That already lives in `NOTES.md` and
  `learning-records/`. A node is a pointer, nothing else.
...
- **A node closes at the Bloom threshold** - roughly 80 to 90 per cent on an independent check,
  format 10 MASTERY CHECK in [PRACTICE-FORMAT.md](./PRACTICE-FORMAT.md). See `SKILL.md`'s THE
  NEXT STEP.
...
- **Building and rebuilding checks fit, when `MISSION.md` states a deadline and a budget.**
  ...
  whichever format survives, the node still closes only through the Bloom threshold check
  above; cutting depth cuts the path there, never the check itself. `core` still does not fit
  after both cuts: stop and ask the user to demote a specific `core` line to `extra` or extend
  the budget. Never drop `core` silently, and never lower the threshold to make it fit - that
  ...
  tension, holding the target fixed and letting time float. Holding the Bloom threshold and
...
- **Numbers are stable ids.** `OBSERVATIONS.md` may point at a node by number. A number is
  assigned once and never reused - nodes are never renumbered. A closed (`done`) node may be
  dropped from the file after a session passes; the next new node takes the next free number.

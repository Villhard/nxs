...
For an applicable packet field with no content, state `none` explicitly, including Conventions, additional rules, directives, requirement sources or saved history. Omission means missing input, not an empty value. Give the ticket and report paths as read-only context and state that only the orchestrator may write them.

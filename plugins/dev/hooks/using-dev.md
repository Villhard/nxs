...
Before acting on any development work - including clarifying questions, reading code, or running a command - check whether a dev command applies, and invoke it with the `Skill` tool first when one does. A command that applies is used, not skipped.

Dispatched as a subagent to execute a specific task? Your task is already scoped; this does not apply to you.

## The rule

The dev commands (`rnd`, `bug`, `plan`, `exec`, `review`, `commit`) are listed with their triggers in the `Skill` tool. Match the work in front of you to a command and invoke it before you start, rather than hand-rolling a workflow a command already owns.

Two triggers are easy to miss because no command name is spoken:

- **Committing working changes** -> `/dev:commit`, which owns the message format, the atomicity rule, and git safety.
...
- "I'll commit this quickly" - a git write pulls `/dev:commit`.

...
Your prompt carries the goal and any ticket paths, with their source: disk or a `git show` of the reviewed version. Use that source and no other. Read `**What to build:**` and every acceptance criterion above `## Implementation`, including both `- [ ]` and `- [x]`. Completion does not remove a requirement. With a goal sentence and no ticket, that sentence is the requirement.
